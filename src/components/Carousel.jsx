...
const smallSizes = ["[350px]", "[420px]", "[290px]", "[385px]", "[390px]"];
const largeSizes = ["[600px]"];
...
const Carousel = ({ images, size }) => {
  ...
  return (
    <div
      className={`h-${sizes[0]} w-${sizes[0]} mx-auto py-8 px-4 relative group lg:h-${sizes[1]} lg:w-${sizes[1]}`}
    >
      <div
        style={{ backgroundImage: `url(${images[currentImageIndex].url})` }}
        className={`h-${sizes[2]} rounded-2xl bg-center bg-cover duration-500 lg:h-${sizes[3]} lg:w-${sizes[4]}`}
      ></div>
      <button
